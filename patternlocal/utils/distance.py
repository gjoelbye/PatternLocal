...
def calculate_distances(
    ...
    if method == "euclidean":
        ...
        return _dtw_distances(X, point)
    else:
        raise ValueError(f"Unknown distance method: {method}")


def _dtw_distances(X: np.ndarray, point: np.ndarray) -> np.ndarray:
    """Calculate DTW distances.

    Requires the tslearn package. Install with:
        pip install patternlocal[timeseries]
    """
    try:
        from tslearn.metrics import dtw
    except ImportError:
        raise ImportError(
            "tslearn is required for DTW distance. "
            "Install it with: pip install patternlocal[timeseries]"
        )
    point_2d = point.flatten()
    distances = np.zeros(X.shape[0])
    for i in range(X.shape[0]):
        distances[i] = dtw(X[i], point_2d)
    return distances
...
def _chebyshev_distances(X: np.ndarray, point: np.ndarray) -> np.ndarray:
    """Calculate Chebyshev (L-inf) distances."""
    point_2d = point.reshape(1, -1)
