...
class LocalCovarianceSolver(LocalSolverBase):
    ...
    def solve(self,
        ...
        try:
            ...
            raise ComputationalError(f"Error computing local covariance patternlocal: {e}")
    
    def _estimate_local_covariance(self, X_train: np.ndarray, point: np.ndarray) -> np.ndarray:
    ...
    def _compute_weighted_covariance(self, points: np.ndarray, weights: np.ndarray) -> np.ndarray:
        ...
        if self.shrinkage_intensity > 0:
            ...
            cov = (1 - self.shrinkage_intensity) * cov + self.shrinkage_intensity * target
