...
class LocalSolverBase(BaseSolver):
    ...
    def _get_analysis_point(self, lime_weights: np.ndarray, lime_intercept: float, instance: np.ndarray) -> np.ndarray:
    ...
    def _get_local_data_and_weights(self, X_train: np.ndarray, point: np.ndarray):
        ...
        if weight_sum == 0:
            raise ComputationalError("Sum of weights is zero. Consider increasing k or adjusting bandwidth.")
        weights /= weight_sum
    ...
    def _get_k_neighbors(self, n_total_samples: int) -> int:
        ...
        if isinstance(self.k_ratio, float):
            ...
            if self.k_ratio <= 0 or self.k_ratio > n_total_samples:
                raise ValueError("k_ratio must be a positive integer less than or equal to the number of samples.")
            k = self.k_ratio
        else:
            raise ValueError("k_ratio must be a float between 0 and 1, or a positive integer.")
