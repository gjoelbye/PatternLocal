...
class ExplainerConfig:
    ...
    def _normalize_configs(self):
        ...
        if isinstance(self.simplification, str):
            self.simplification = SimplificationConfig(
                method=self.simplification,
                params=self.simplification_params
            )
        elif isinstance(self.simplification, dict):
            self.simplification = SimplificationConfig(**self.simplification)
        elif not isinstance(self.simplification, (BaseSimplification, SimplificationConfig)):
        ...
        if hasattr(self.simplification, 'method'):
            if self.simplification.method == 'superpixel' and self.lime_params.mode == 'tabular':
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'simplification': getattr(self.simplification, 'method', self.simplification),
            'solver': getattr(self.solver, 'method', self.solver),
            'lime_params': self.lime_params.to_dict() if hasattr(self.lime_params, 'to_dict') else self.lime_params,
            'simplification_params': getattr(self.simplification, 'params', self.simplification_params),
