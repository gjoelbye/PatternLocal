...
class ParameterValidator:
    ...
    @staticmethod
    def validate_predict_function(predict_fn: Callable,
                                 test_input: np.ndarray,
                                 expected_output_shape: Optional[tuple] = None) -> None:
    ...
    def validate_lime_params(mode: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'num_samples' in validated_params:
            if not isinstance(validated_params['num_samples'], int) or validated_params['num_samples'] <= 0:
                raise ValidationError("num_samples must be a positive integer")
        
        if 'kernel_width' in validated_params and validated_params['kernel_width'] is not None:
            if not isinstance(validated_params['kernel_width'], (int, float)) or validated_params['kernel_width'] <= 0:
    ...
    @staticmethod
    def validate_solver_inputs(lime_weights: np.ndarray,
        ...
        if X_train.shape[1] != instance.shape[0]:
            raise ValidationError("X_train feature dimension must match instance length")
    
    @staticmethod
    def validate_image_params(image_shape: tuple,
                             instance: np.ndarray,
                             simplification_params: Dict[str, Any]) -> None:
