...
def project_point_onto_hyperplane(normal_vector: np.ndarray,
                                 intercept: float,
                                 point: np.ndarray,
                                 normalize_normal: bool = True) -> np.ndarray:
...
def project_point_onto_line(line_point: np.ndarray,
...
def project_points_onto_subspace(basis_vectors: np.ndarray,
                                points: np.ndarray,
...
def orthogonal_complement_projection(subspace_basis: np.ndarray,
...
def gram_schmidt_orthogonalize(vectors: np.ndarray,
    ...
    try:
        ...
        for i in range(n_vectors):
            ...
            if vector_norm < 1e-10:
                raise ComputationalError(f"Vector {i} is linearly dependent on previous vectors")
...
def distance_point_to_hyperplane(normal_vector: np.ndarray,
                                intercept: float,
...
def distance_point_to_line(line_point: np.ndarray,
                          line_direction: np.ndarray,
                          point: np.ndarray) -> float:
...
def reflect_point_across_hyperplane(normal_vector: np.ndarray,
                                   intercept: float,
                                   point: np.ndarray) -> np.ndarray:
