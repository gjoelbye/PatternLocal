...
def grid_segmentation(instance, dims=(64, 64), grid_rows=8, grid_cols=8):
    ...
    assert h % grid_rows == 0, f"Image height {h} not divisible by grid_rows {grid_rows}"
...
def slic_segmentation(instance, dims=(64, 64), n_segments=200, compactness=8, sigma=0):
    ...
    seg_map = slic(resized, n_segments=n_segments, compactness=compactness, sigma=sigma, start_label=0)
