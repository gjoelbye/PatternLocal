...
def biweight_kernel(distances: np.ndarray, bandwidth: float) -> np.ndarray:
    ...
    weights[inside_support] = (15/16) * (1 - normalized_distances[inside_support] ** 2) ** 2
    return weights
...
def tricube_kernel(distances: np.ndarray, bandwidth: float) -> np.ndarray:
    ...
    inside_support = normalized_distances <= 1
    weights = np.zeros_like(distances)
    weights[inside_support] = (70/81) * (1 - normalized_distances[inside_support] ** 3) ** 3
    return weights
...
def cosine_kernel(distances: np.ndarray, bandwidth: float) -> np.ndarray:
    ...
    inside_support = normalized_distances <= 1
    weights = np.zeros_like(distances)
    weights[inside_support] = (np.pi/4) * np.cos(np.pi/2 * normalized_distances[inside_support])
    return weights
