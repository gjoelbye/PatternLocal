...
class TabularModeStrategy(LimeModeStrategy):
    ...
    def create_explainer(self, 
                        X_train: np.ndarray, 
                        lime_params: Dict[str, Any],
                        random_state: Optional[int] = None) -> lime_tabular.LimeTabularExplainer:
        """Create tabular LIME explainer."""
        validated_params = ParameterValidator.validate_lime_params('tabular', lime_params)
...
class ImageModeStrategy(LimeModeStrategy):
    ...
    def create_explainer(self, 
                        X_train: np.ndarray, 
                        lime_params: Dict[str, Any],
                        random_state: Optional[int] = None) -> lime_image.LimeImageExplainer:
    ...
    def generate_explanation(self,
        ...
        if not hasattr(simplification, 'image_shape') or simplification.image_shape is None:
            raise ExplanationError("SuperpixelSimplification must have image_shape defined for image mode")
        ...
        # Get parameters
        labels = kwargs.get('labels', [1])  # Default to class 1 for binary classification
...
class StrategyFactory:
    ...
    def create_strategy(cls, mode: str) -> LimeModeStrategy:
        ...
        if mode not in cls._strategies:
            ...
            raise ValidationError(f"Unsupported LIME mode: {mode}. Available: {available}")
        
        return cls._strategies[mode]()
    
    @classmethod
    def auto_detect_strategy(cls, 
                           simplification: BaseSimplification,
                           lime_params: Dict[str, Any]) -> LimeModeStrategy:
