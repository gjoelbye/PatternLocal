...
class SuperpixelSimplification(BaseSimplification):
    ...
    def fit(self, X_train: np.ndarray, **kwargs) -> 'SuperpixelSimplification':
        ...
        if self.image_shape is None:
            raise ValueError("image_shape must be provided either in params or as kwarg")
    ...
    def _grid_segmentation(self) -> np.ndarray:
        ...
        if h % self.grid_rows != 0:
            raise ValueError(f"Image height {h} not divisible by grid_rows {self.grid_rows}")
        if w % self.grid_cols != 0:
            raise ValueError(f"Image width {w} not divisible by grid_cols {self.grid_cols}")
    ...
    def inverse_transform_weights(self, weights: np.ndarray) -> np.ndarray:
        ...
        """
        if not self.is_fitted:
            raise ValueError("SuperpixelSimplification must be fitted before inverse_transform")
    ...
    def create_predict_function(self, original_predict_fn: Callable) -> Callable:
        ...
        """
        if not self.is_fitted:
            raise ValueError("SuperpixelSimplification must be fitted before creating predict function")
