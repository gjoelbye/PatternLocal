...
class NoSimplification(BaseSimplification):
    ...
    def transform_instance(self, instance: np.ndarray) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValidationError("NoSimplification must be fitted before transforming instances")
    ...
    def transform_training_data(self, X_train: np.ndarray) -> np.ndarray:
        ...
        """
        if not self.is_fitted:
            raise ValidationError("NoSimplification must be fitted before transforming data")
    ...
    def get_feature_names_out(self, feature_names_in: Optional[list] = None) -> Optional[list]:
