...
class LowRankSimplification(BaseSimplification):
    ...
    def transform_instance(self, instance: np.ndarray) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValidationError("LowRankSimplification must be fitted before transform")
    ...
    def inverse_transform_weights(self, weights: np.ndarray) -> np.ndarray:
        ...
        """
        if not self.is_fitted:
            raise ValidationError("LowRankSimplification must be fitted before inverse_transform")
    ...
    def create_predict_function(self, original_predict_fn: Callable) -> Callable:
        ...
        """
        if not self.is_fitted:
            raise ValidationError("LowRankSimplification must be fitted before creating predict function")
