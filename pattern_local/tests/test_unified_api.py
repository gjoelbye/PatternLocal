...
class TestUnifiedAPI:
    ...
    def test_auto_detection_tabular(self, small_tabular_data, predict_fn_tabular, random_state):
    ...
    def test_auto_detection_image(self, small_image_data, predict_fn_image, random_state):
    ...
    def test_explicit_mode_override(self, small_image_data, predict_fn_image, random_state):
    ...
    def test_all_solvers_work_with_both_modes(self, small_tabular_data, small_image_data,
                                             predict_fn_tabular, predict_fn_image, solver, random_state):
    ...
    def test_consistent_api_across_modes(self, small_tabular_data, small_image_data,
                                        predict_fn_tabular, predict_fn_image, random_state):
        ...
        explainer_img = PatternLocalExplainer(
            simplification='superpixel',
            solver='local_covariance',
            simplification_params={'image_shape': image_shape, 'method': 'grid', 'grid_rows': 2, 'grid_cols': 2},
            random_state=random_state
        )
        
        # Both should have the same methods and properties
        for attr in ['fit', 'explain_instance', 'mode', 'simplification_method', 'solver_method']:
        ...
        explanation_tab = explainer_tab.explain_instance(X_tab[0], predict_fn_tabular, X_tab)
        explanation_img = explainer_img.explain_instance(X_img[0], predict_fn_image, X_img, labels=[1])
        
        # Both should return the same structure
        expected_keys = {'pattern_weights', 'lime_weights', 'lime_intercept', 'local_exp'}
    ...
    def test_error_handling(self, random_state):
        ...
        with pytest.raises(ValueError, match="Image mode requires SuperpixelSimplification"):
            explainer = PatternLocalExplainer(
                simplification='none',
