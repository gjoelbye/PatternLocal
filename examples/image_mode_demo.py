...
def create_sample_image_data(n_samples=1000, image_shape=(28, 28), n_classes=2, random_state=42):
...
def demo_unified_api():
    ...
    print(f"Pattern weights range: [{pattern_weights.min():.3f}, {pattern_weights.max():.3f}]")
...
def visualize_explanations(instance, pattern_weights, lime_weights, image_shape):
    ...
    im1 = axes[1].imshow(lime_image, cmap='RdBu', vmin=-np.abs(lime_image).max(), vmax=np.abs(lime_image).max())
    ...
    im2 = axes[2].imshow(pattern_image, cmap='RdBu', vmin=-np.abs(pattern_image).max(), vmax=np.abs(pattern_image).max())
...
def demo_mode_comparison():
    ...
    print(f"  ✓ Generated explanation with shape: {explanation_tab['pattern_weights'].shape}")
    ...
    print(f"  ✓ Generated explanation with shape: {explanation_img['pattern_weights'].shape}")
...
def demo_different_solvers():
    ...
    for solver_name in solvers:
        ...
        pattern_weights = explanation['pattern_weights']
        print(f"  ✓ Mode: {explainer.mode} | Weights range: [{pattern_weights.min():.3f}, {pattern_weights.max():.3f}]")
