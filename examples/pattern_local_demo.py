...
def demo_lowrank_simplification():
    ...
    configs = [
        ...
        {'simplification': 'lowrank', 'simplification_params': {'n_components': 10}, 'name': 'Low-rank (10 components)'},
        {'simplification': 'lowrank', 'simplification_params': {'n_components': 0.95}, 'name': 'Low-rank (95% variance)'}
    ]
...
def demo_custom_parameters():
    ...
    print(f"  Pattern weights magnitude: {np.linalg.norm(explanation['pattern_weights']):.3f}")
    print(f"  LIME weights magnitude: {np.linalg.norm(explanation['lime_weights']):.3f}")
